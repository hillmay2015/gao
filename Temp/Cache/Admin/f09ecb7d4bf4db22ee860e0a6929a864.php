...
<form action="<?php echo C('cfg_app'); echo U(GROUP_NAME.'/Admin/edit');?>" method="post">
    <table width="100%" border="0" cellpadding="6" cellspacing="0" class="tableBasic">
        ...
        <tr>
            ...
            <td>
                <label>
                    <?php if(data['is_super']==1): ?><input type="radio" name="gid" value="1"  if($data.gid==1){ checked }>
                        普通管理员
                        <input type="radio" name="gid" value="2"  if($data.gid==2){ checked }>
                        渠道商<?php endif; ?>
                    <?php if(data['is_super']==0): ?><input type="radio" name="gid" value="3" if($data.gid==1){ checked }>
                        推广员<?php endif; ?>
                </label>
            </td>
        </tr>
        ...
        <!-- 超级管理员 编辑渠道商 默认模板为 0-->
        <?php if(data['is_super']==1): ?><input type="hidden" value="0" name="tpl" checked><?php endif; ?>
        <!-- 渠道商 编辑推广员 模板需要选择-->
        <?php if($data['is_super']==0): ?><tr>
